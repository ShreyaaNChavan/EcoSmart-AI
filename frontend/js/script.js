...
function predictNoise() {
    ...
    // Loading state
    box.innerHTML = `<div class="result-detail">Predicting...</div>`;
    box.classList.add('show');

    fetch("http://127.0.0.1:5000/predict_noise", {
        method: "POST",
        headers: {
        ...
        .then(data => {
            ...
            let color;
            if (data.level === "Low") color = "var(--accent)";
            else if (data.level === "Medium") color = "var(--accent3)";
            else color = "var(--warn)";
...
const bioData = {
    heavy_metals: {
        ...
        time: { low: '3–6 months', medium: '8–14 months', high: '18–36 months' },
...
function predictBio() {
    ...
    if (!d) {
        ...
        console.error("Wrong key:", pollutant);
        return;
    }
    ...
    box.innerHTML = `
        ...
        <div class="result-action">${d.notes}</div>
    `;
...
// Fetch items from backend once
async function loadItems() {
    try {
        const res = await fetch("http://127.0.0.1:5000/get_items");
...
async function predictWaste() {
    ...
    try {
        ...
        // API call
        const res = await fetch("http://127.0.0.1:5000/predict_waste", {
            method: "POST",
        ...
        // ✅ Impact (basic for now)
        const impactEl = document.getElementById("impactMeter");
        if (impactEl) {
            impactEl.innerHTML = `
                <div class="impact-row">
                    <span>Impact Level:</span>
                    <span style="color:var(--accent)">${data.impact || "Moderate"}</span>
                </div>
            `;
        }
        ...
        loader.style.display = "none";
        resultBox.innerHTML = `<span style="color:red">Server error. Try again.</span>`;
